package TremolZFP;
public class DailyPObyOperatorRes {
   /**
    *Symbols from 1 to 20 corresponding to operator's number
    */
    public Double OperNum;
    public Double getOperNum() {
       return OperNum;
    }
    protected void setOperNum(Double value) {
       OperNum = value;
    }

   /**
    *Up to 13 symbols for the PO by type of payment 0
    */
    public Double AmountPO_Payment0;
    public Double getAmountPO_Payment0() {
       return AmountPO_Payment0;
    }
    protected void setAmountPO_Payment0(Double value) {
       AmountPO_Payment0 = value;
    }

   /**
    *Up to 13 symbols for the PO by type of payment 1
    */
    public Double AmountPO_Payment1;
    public Double getAmountPO_Payment1() {
       return AmountPO_Payment1;
    }
    protected void setAmountPO_Payment1(Double value) {
       AmountPO_Payment1 = value;
    }

   /**
    *Up to 13 symbols for the PO by type of payment 2
    */
    public Double AmountPO_Payment2;
    public Double getAmountPO_Payment2() {
       return AmountPO_Payment2;
    }
    protected void setAmountPO_Payment2(Double value) {
       AmountPO_Payment2 = value;
    }

   /**
    *Up to 13 symbols for the PO by type of payment 3
    */
    public Double AmountPO_Payment3;
    public Double getAmountPO_Payment3() {
       return AmountPO_Payment3;
    }
    protected void setAmountPO_Payment3(Double value) {
       AmountPO_Payment3 = value;
    }

   /**
    *Up to 13 symbols for the PO by type of payment 4
    */
    public Double AmountPO_Payment4;
    public Double getAmountPO_Payment4() {
       return AmountPO_Payment4;
    }
    protected void setAmountPO_Payment4(Double value) {
       AmountPO_Payment4 = value;
    }

   /**
    *Up to 13 symbols for the PO by type of payment 5
    */
    public Double AmountPO_Currency;
    public Double getAmountPO_Currency() {
       return AmountPO_Currency;
    }
    protected void setAmountPO_Currency(Double value) {
       AmountPO_Currency = value;
    }

   /**
    *5 symbols for the total number of operations
    */
    public Double NumPO;
    public Double getNumPO() {
       return NumPO;
    }
    protected void setNumPO(Double value) {
       NumPO = value;
    }
}
